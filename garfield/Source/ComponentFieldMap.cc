...
void ComponentFieldMap::Jacobian5(const Element& element, const double u,
  ...
  det =
      (-twou * u2 * ((n2.x + n3.x - two6x) * (n0.y + n1.y - two4y) -
                     (n0.x + n1.x - two4x) * (n2.y + n3.y - two6y)) +
       twov * v2 * (-((n0.x + n3.x - two7x) * (n1.y + n2.y - two5y)) +
...
int ComponentFieldMap::Coordinates13(const double x, const double y,
  ...
  t1 = (fabs(t1) < 1E-10) ? fabs(t1) : t1; //change the sign if it's negtive
  t2 = (fabs(t2) < 1E-10) ? fabs(t2) : t2; 
  t3 = (fabs(t3) < 1E-10) ? fabs(t3) : t3; 
  t4 = (fabs(t4) < 1E-10) ? fabs(t4) : t4;

  t1 = (fabs(t1-1) < 1E-10) ? 1 - fabs(t1) : t1; //change to 1 minus if it's larger than 1
  t2 = (fabs(t2-1) < 1E-10) ? 1 - fabs(t2) : t2; 
  t3 = (fabs(t3-1) < 1E-10) ? 1 - fabs(t3) : t3; 
  t4 = (fabs(t4-1) < 1E-10) ? 1 - fabs(t4) : t4; 
