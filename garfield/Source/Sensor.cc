...
// added by LIUQ
//
bool Sensor::WriteSensorFile(const std::string& filename)
{
  m_rfile = new TFile(filename.c_str(),"RECREATE");
  if (!m_rfile) {
    std::cerr << m_className << "::WriteSensorFile:\n"
              << "    Cannot open file " << filename << ".\n";
    return false;
  }

  m_rtree = new TTree("tree","trees");
  m_rtree->Branch("qhit", &qhit);
  m_rtree->Branch("thit", &thit);
  m_rtree->Branch("dthit",&dthit);
  m_rtree->Branch("xhit", &xhit);
  m_rtree->Branch("yhit", &yhit);
  m_rtree->Branch("zhit", &zhit);
  m_rtree->Branch("vxhit",&vxhit);
  m_rtree->Branch("vyhit",&vyhit);
  m_rtree->Branch("vzhit",&vzhit);
